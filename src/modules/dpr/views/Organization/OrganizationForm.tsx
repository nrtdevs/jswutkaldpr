...
const OrganizationForm = () => {
    ...
    const [createOrganization, result] = useCreateOrUpdateOrganizationMutation()
    const [getOrganization, { data, isSuccess, isLoading }] = useLoadOrganizationDetailsByIdMutation()
    ...
    return (
        <>
            <Header
                onClickBack={() => nav(-1)}
                goBackTo
                title={isValid(params?.id) ? FM('organization-update') : <>{FM('organization-create')}</>}
            ></Header>
            {loading ? (
                <>
                    <Row>
