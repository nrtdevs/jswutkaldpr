...
export const HeaderManagement = createApi({
  reducerPath: 'HeaderManagement',
  baseQuery: axiosBaseQuery(),
  endpoints: (builder) => ({
    loadHeader: builder.mutation<ResponseTypeList, RequestType>({
      query: (a) => ({
        jsonData: a?.jsonData,
        params: { page: a?.page, per_page_record: a?.per_page_record },
        method: 'post',
        path: ApiEndpoints.headerList
      })
    }),
    createOrUpdateHeader: builder.mutation<ResponseType, DPR>({
      query: (jsonData) => ({
        jsonData,
        showSuccessToast: true,
        method: isValid(jsonData?.id) ? 'put' : 'post',
        path: isValid(jsonData?.id) ? ApiEndpoints?.headerAddUpdate + '/' + jsonData?.id : ApiEndpoints.headerAddUpdate
      })
    }),
    ...
    loadHeaderDetailsById: builder.mutation<ResponseType, DPR>({
      query: (a) => ({
        ...
        method: 'get',
        params: { page: a?.page, per_page_record: a?.per_page_record },
        path: ApiEndpoints.headerAddUpdate + '/' + a?.id
      })
    }),
  })
})
export const {
  useLoadHeaderMutation,
  useCreateOrUpdateHeaderMutation,
  useDeleteHeaderByIdMutation,
  useLoadHeaderDetailsByIdMutation
} = HeaderManagement
